...
function ConfigContainer(props) {
	...
	const getTypes = async () => {
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t("ASABConfig|Unable to get data for tree menu", {error: e?.response?.data?.message}), 30);
			return;
		}
	}
	...
	const getConfigs = async (typeId) => {
		...
		try {
			...
			console.error(e);
			props.app.addAlert("warning", t(`ASABConfig|Unable to get schema. Try to reload the page`, {type: typeId}), 30);
			return;
		}
	}
